...
import { EnvironmentModuleNode, isRunnableDevEnvironment, type Plugin, type ViteDevServer } from 'vite'
  ...
  return {
    ...
    hotUpdate(ctx) {
      if (isImported(ctx.file)) {
        ...
        this.environment.hot.send({ type: "custom", event: "vike-server:close-server", data: "GUY" });
